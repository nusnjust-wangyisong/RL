...
| HER / 课程学习 / 稳定性约束 / 动作平滑 / 精密伺服消融 | 已完成 | `runs/results/ablation_report.md` |
...
该结果说明，HER 与课程学习/稳定性约束主要保证 5 mm 到达能力和随机目标泛化，但单独依靠训练阶段稳定性奖励并不足以显著降低受迫振动指标。动作平滑可以降低部分 jerk 和动作变化量，但最终误差改善有限。最终加入多目标稳定精密伺服后，最终误差和稳定性指标同步大幅下降。也就是说，最终方法的优势不是单个模块带来的，而是训练阶段约束和执行阶段精密伺服共同作用的结果。
