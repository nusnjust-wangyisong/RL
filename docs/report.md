...
本项目以 Franka Panda 七自由度机械臂为对象，在 PyBullet 仿真环境中构建高精度 Reach 任务。相较中期阶段的 20 mm 精度目标，本阶段将主线目标提升至 5 mm，并增加接触变力扰动，用末端加速度 RMS、峰峰值、jerk 和振动指数评价稳定性。
...
在接触变力扰动下，完整方法不仅保持毫米级定位精度，还同时降低了末端加速度、峰峰值、jerk 和动作变化量，体现高精度与高稳定性的协同。
...
该结果表明，最终方法在 6 自由度 IRB120 上仍保持 5 mm 高精度，并且在达到加工精度的前提下稳定性指标优于 DDPG、TD3、SAC、TQC 和 TD3+HER 基线。IRB120 扩展验证了方法的跨构型泛化能力：通过重构动作空间、观测空间和 IK 伺服映射，可迁移到不同自由度机械臂，而不依赖 Panda 单一结构。
