...
在末端进入目标点 20 mm 邻域（模拟刀具接触后才受力）沿 +z 方向施加：
...
| 方向 | +z（`[0,0,1]`） | 沿法向/进给的接触反力 |
| `F0` | 0 N | 无静态预压（可扩展） |
| `A`（light/medium/strong） | 2 / 5 / 10 N | 受迫振动幅值 |
| `σ`（light/medium/strong） | 0.2 / 0.5 / 1.0 N | 随机扰动（材料不均、颤振噪声） |
| `f` | 20 Hz | 周期性切削/磨削激励 |

激励频率 `f=20 Hz` 与本文控制频率（约 20 Hz）同量级，接近控制带宽上限，是对稳定性最不利、也最具区分度的工况。`F(t)` 经 §3.1 的 `F_ext` 项进入动力学方程，故稳定性评价反映真实的力—振动响应。
...
| 末端振动指数 | 综合振动强度 |
| 目标邻域误差波动 std | 末端首次进入 5 mm 后剩余步的定位误差标准差，直接度量“稳定保持”能力 |

各指标越小，表示末端运动越平稳、受迫振动响应越弱。其中“目标邻域误差波动 std”为评价协议指标，统计口径见 §6。
...
稳定性中的“目标邻域误差波动 std”按如下口径统计：在每个 episode 中，从末端首次进入 5 mm 邻域的时间步起，取其后所有步的定位误差序列，计算其标准差，再对多个 episode 取平均。该指标与 Hold@5mm 互补——Hold@5mm 给出停留在邻域内的时间占比，误差波动 std 给出停留期间的定位抖动幅度，二者共同刻画“稳定保持”能力。
